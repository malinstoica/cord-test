...
/**
 * Helper function to validate a request body, given a schema to be followed.
 * 
 * @throws throw an error code with a relevant error message
 * 
 * @param {object} schema type schema for the parameters
 * @param {object} request full request data object
 */
const checkAndValidateRequestBodyFor = (schema, request) => {
  ...
  let schemaForRequestParameters = schema.filter(parameter => givenParameters.includes(parameter.name) || (parameter.schema && parameter.schema.properties));
